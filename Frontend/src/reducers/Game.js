...
export const Game = (state = initialState, action) => {
    switch (action.type) {
        ...
        case DRAW_CARD:
            var { card: pickedCard, deck: postDrawDeck } = pickCardFromDeck(
                state.deck,
                state.cards
            )
            return {
                ...
                hands: {
                    playerHand:
                        action.turn === Turn.PLAYER ||
                        state.turn === Turn.PLAYER
                            ? draw(pickedCard, state.hands.playerHand)
                            : state.hands.playerHand,
                    enemyHand:
                        action.turn === Turn.ENEMY || state.turn === Turn.ENEMY
                            ? draw(pickedCard, state.hands.enemyHand)
                            : state.hands.enemyHand
                },
                deck: postDrawDeck
            }
        ...
        case SELECT_RANDOM_CARD_FROM_HAND:
            ...
            return {
                ...state,
                cards: selectCard(
                    randomH(currentHand, state.cards),
                    state.cards
                )
            }
        ...
        case PLAY_CARD_FROM_DECK:
            ...
            } = pickCardFromDeck(state.deck, state.cards)
        ...
        case PLAY_RANDOM_CARD_FROM_HAND:
            ...
            return {
                ...state,
                board: play(
                    selectedRandomCardFromHand,
                    shuffle.pick(shuffle([0, 1, 2, 3, 4, 5])),
                    state.board
                ),
                hands: {
                    playerHand:
        ...
        case UPDATE_CURRENT_GAME_STATE:
            return {
                ...state,
                status: isEmptyD(state.deck)
                    ? GameResult.TIE
                    : isEmptyH(state.hands.playerHand)
                        ? GameResult.WINNER
                        : isEmptyH(state.hands.enemyHand)
                            ? GameResult.LOSER
                            : GameResult.PLAYING
            }
