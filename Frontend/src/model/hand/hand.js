...
export const includes = (card, { slots }) =>
    any(partial(includesS, [card]), slots)

export const draw = (card, hand) => set(card, find(isEmptyS, slots(hand)), hand)

export const remove = (card, hand) =>
    clear(find(partial(includesS, [card]), slots(hand)), hand)

export const random = ({ slots }, cards) =>
    getS(pick(filter(complement(isEmptyS), slots)), cards)
