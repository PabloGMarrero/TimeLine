...
import { isEmpty, head, lte, equals, tail, length, update, indexOf, map, find, and } from 'ramda'
...
export const reveal = (card, cards) => updateCard(card, { visible: true }, cards)

export const deselectCards = (cards) => map(_ => ({ ..._, selected: false }), cards)

export const selectCard = (card, cards) => updateCard(card, { selected: true }, deselectCards(cards))
...
export const timelineHasAValidPeriod = (cards) =>
    isEmpty(cards) ? true :
        equals(size(cards), 1) ? true :
            and(evaluateCards(head(cards), head(tail(cards))), timelineHasAValidPeriod(tail(cards)))
